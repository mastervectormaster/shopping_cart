...
def add_amount(order):
    order_with_amounts = order
    amounts = [
        item.amount for item in OrderProduct.objects.filter(order_id=order["id"])
    ]
    order_with_amounts["amounts"] = amounts
    return order_with_amounts
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        response = map(add_amount, super().list(request).data)
    ...
    def add_product(self, request, pk=None):
        ...
        OrderProduct.objects.create(product=product, order=order, amount=amount)
